...
export class SequenceConservationComponent extends React.Component<ISequenceConservationComponentProps, 
  ...
  handleSvgHover(isHovered: boolean) {
    console.log('new ishovered:' + isHovered);
    console.log('current ishovered:' + this.state.isHovered);
    this.setState({
      isHovered: isHovered
    });
  }
   ...
   */
  computeEntropy(allLetters: string[], pk: {[letter: string]: number}, qk: {[letter: string]: number}, doLog?: boolean){
  ...
  createConservationCountGs() {
    ...
    const positionalLetterCounts = this.props.alignment.getPositionalLetterCounts(true, allLetters);
    const backgroundLetterCounts = this.props.alignment.getGlobalAlphaLetterCounts(true, allLetters);

    //const maximumEntropy = this.computeEntropy(allLetters, backgroundLetterCounts, backgroundLetterCounts);
    const entropies = Array.from(positionalLetterCounts).map(([position, letterCounts]) => {
      return this.computeEntropy(allLetters, letterCounts, backgroundLetterCounts)
    });
    ...
    normalizedEntropies.forEach((normalizedEntropy, idx) => {
      toReturn.push(
        <g transform={`translate(${idx*this.props.characterWidth+this.props.characterWidth/2},0)`} key={idx}>
          <rect width={`${this.props.characterWidth/2}`} 
                height={`${normalizedEntropy}`}></rect>
        </g>
    ...
    for(let i = 0; i < maxSeqLength; i++){
      toReturn.push(
        <g transform={`translate(${i*this.props.characterWidth},0)`} key={i}>
          <rect width={`${this.props.characterWidth/2}`} 
                height={`${gapCounts[i] / numSequences * 100}`}></rect>
        </g>
      )
    }
    return toReturn;
  ...
  divLoaded(e: HTMLDivElement | null){
    if(e && !this.scrollerDiv){ //only run if editor not loaded
      this.scrollerDiv = e;
  ...
  render() {
    ...
    const hoveredClass = this.state.isHovered ? "conservation hovered" : "conservation";
