...
export default class ScrollSync {
    ...
    private _groupToAceEditors: { [key: string]: number } = {};
    private _groupToElements: { [key: string]: Element } = {};
    ...
    _handleHorizontalScrollEvent(scroller: HTMLElement | Ace.Editor, groupName: string, newScrollLeft: number){
        ...
        if ('renderer' in scroller){ //the node being scrolled is an ace editor
            scrollerFractionScrolled = newScrollLeft / this._getMaxScrollWidth(scroller);
        }
        ...
        for(var i = 0; i < group.scrollers.length; i++){
            ...
            if (targetScroller !== scroller){
                if ('renderer' in targetScroller){ //its another ace editor
                    const targetMaxWidth = this._getMaxScrollWidth(targetScroller);
                    const proportionalScrollLeft = Math.round(scrollerFractionScrolled * targetMaxWidth);
                    if (proportionalScrollLeft !== targetScroller.session.getScrollLeft()){
                ...
                else{ //its an html element
                    const targetMaxWidth = targetScroller.scrollWidth - targetScroller.offsetWidth;
                    const proportionalScrollLeft = Math.round(scrollerFractionScrolled * targetMaxWidth);
    ...
    setScrollerGroup(groupName: string, scrollType: ScrollType){
        this._groupScrollpropsHM[groupName] = {
            scrolltype: scrollType,
            scrollers: this._groupScrollpropsHM[groupName] ? this._groupScrollpropsHM[groupName].scrollers : []
        }
    }
    ...
    registerScroller(scroller: HTMLElement | Ace.Editor, groupName: string){
        ...
        if ('renderer' in scroller){ //the scroller is an ace editor
            if (group.scrolltype === ScrollType.both || ScrollType.horizontal){
                scroller.session.on("changeScrollLeft", newScrollLeft => {
                    ...
                    this._handleHorizontalScrollEvent(scroller, groupName, Math.round(scroller.session.getScrollLeft()));
                });
            }
        ...
        else{ //the scroller is an html element
            if (group.scrolltype === ScrollType.both || ScrollType.horizontal){
                scroller.onscroll = (e) => {
                    if (e.target){
                        this._handleHorizontalScrollEvent(scroller, groupName, (<HTMLElement>e.target).scrollLeft);
                    }
                };
            }
            if (group.scrolltype === ScrollType.both || ScrollType.vertical){
